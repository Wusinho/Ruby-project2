...
def my_inject (op = nil, param = nil)
  if (!op.nil? && !param.nil?) && op.is_a?(String)
    my_each {|item| op.send(item, param)}
end
if block_given? && !param.nil?
  my_each { |item| op = op.nil? ? item : op.send(param, item) } 
else
  my_each { |ele| arg1 = arg1.nil? ? item : yield(op, item) }
end
puts op
end
